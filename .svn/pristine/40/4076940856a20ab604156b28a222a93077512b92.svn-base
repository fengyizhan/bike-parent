...
//				$scope.dialog.hasUpdateDataAuthorityPower = $scope.operator.setting.hasUpdateDataAuthorityPower;
			...
			$scope.auth = function(event, target){
				...
				$http.get('api/information/authority/authoritiesTree/' + target.username).success(function(response){
					dataAuthorityTree = $.fn.zTree.init($("#dataAuthorityTree"), {
						...
						callback: {
							onCheck: function(event, treeId, treeNode){
								...
								$http({'method' : treeNode.checked?'PUT':'DELETE', 'url' : 'api/information/authority/authoritiesTree/' + target.username + "/" + treeNode.id+ "/" +treeNode.level}).success(function(response){});
							}
						}
					}, response);
