...
public class UserService implements UserServiceInterface {
	...
	public Result userCheck(LoginModel loginModel)
	{
		...
		try {
			...
			try 
			{//初始化扩展信息
				User driver=driverService.getDriverByUsername(username_mobile);
				if(driver==null)
				{
					try{
						/**
						...
						User driver_user=driverService.addDriver(user);
						result.getData().put("user",driver_user);
					}catch(Exception e)
					{
						result.setSuccess(false);
				...
				{
					result.getData().put("user",driver);
				}
			...
			try 
			{//初始化钱包信息
				WalletUser walletUser=walletUserService.getWalletUserByUsername(username_mobile);
				if(walletUser==null)
				{
					try{
						/**
						 ...
						 */
						WalletUser wallet_User=new WalletUser();
						wallet_User.setUsername(username_mobile);
						wallet_User.setBalance(0f);
						walletUserService.addWalletUser(wallet_User);
						result.getData().put("wallet",wallet_User);
					}catch(Exception e)
					{
						result.setSuccess(false);
				...
				{
					result.getData().put("wallet",walletUser);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
