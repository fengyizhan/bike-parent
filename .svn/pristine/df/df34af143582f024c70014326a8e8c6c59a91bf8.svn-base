...
public class Protocol0200Handler implements ProtocolHandler<Received0200> {
	...
	public Message execute(Channel channel, Received received) throws Exception {
		...
		if(received0200.getBody().getElectricity()<= 20){
			pointvector.setState(State.LOWPOWER);
		} else {
			pointvector.setState(State.ONLINE);
		}
		...
		String parkId = String.valueOf(received0200.getHeader().getTerminalId());
		Park park = parkOperator.get(RedisKey.PARKS, parkId);
